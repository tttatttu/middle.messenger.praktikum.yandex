...
export abstract class Block1 {
    ...
    constructor(tagName: string, config: BlockProps, template: string, children = []) {
        ...
        eventBus.emit(Block1.EVENTS.INIT);
    }

    private _registerEvents(eventBus): void {
        eventBus.on(Block1.EVENTS.INIT, this.init.bind(this));
        eventBus.on(Block1.EVENTS.FLOW_CDM, this._componentDidMount.bind(this));
        eventBus.on(Block1.EVENTS.FLOW_CDU, this._componentDidUpdate.bind(this));
        eventBus.on(Block1.EVENTS.FLOW_RENDER, this._render.bind(this));
    }
    ...
    init(): void {
        ...
        this._eventBus().emit(Block1.EVENTS.FLOW_CDM);
    }

    private _componentDidMount(): void {
        this.componentDidMount();
        this._eventBus().emit(Block1.EVENTS.FLOW_RENDER);
    }
    ...
    private _renderChildren(): void {
        ...
        this.children.forEach(child => {
            child._eventBus().emit(Block1.EVENTS.FLOW_CDU);
    ...
    private _makePropsProxy = (props): object => {
        ...
        return new Proxy(props, {
            get(target, prop) {
                ...
                self._eventBus().emit(Block1.EVENTS.FLOW_CDU);
            ...
            set(target, prop, value) {
                ...
                self._eventBus().emit(Block1.EVENTS.FLOW_CDU, {...target}, target);
                return true;
            },
            deleteProperty() {
